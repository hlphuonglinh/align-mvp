import type { PlanInputs, PlanOutputs, ReliabilityWindow, GovernanceDecision, ModeGovernanceDecision } from './types.js';
...
export function computePlan(inputs: PlanInputs): PlanOutputs {
  ...
  let modeDecisions: ModeGovernanceDecision[] | undefined;

  // New-style governor evaluation (if profile and dayISODate provided)
  if (inputs.dayISODate) {
    modeDecisions = evaluateDay({
      profile: inputs.profile ?? null,
      busyBlocks: inputs.busyBlocks ?? [],
      baselineWindows: inputs.baselineWindows ?? [],
      dayISODate: inputs.dayISODate,
    });
  }

  // Legacy chronotype handling (for backwards compatibility)
  ...
  return { reliabilityWindows, decisions, modeDecisions };
}
